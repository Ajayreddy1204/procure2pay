...
<style>
    ...
    /* Hide streamlit button default styling for circle buttons */
    ...
    /* Style for clickable invoice circle button */
    ...
    .circle-button-container button:hover {
    ...
    .circle-button-container.selected button {
    ...
    .circle-button-container.selected button:hover {
...
# Helper: Render single invoice card with clickable circle

# ------------------------------------------------------------

def render_invoice_card(row, status_label, status_class, card_key, is_selected=False):

    """Render a single invoice card with clickable invoice number circle."""
    ...
    # Build the circle text - show full invoice number in two lines

    if inv_bottom:

        circle_text = f"{inv_top}<br><span style='font-size: 1.2rem;'>{inv_bottom}</span>"
...
<!-- Circle placeholder - button will overlay -->
</div>
            ...
            with cols[col_idx]:
                ...
                with card_container:
                    ...
                    with card_col1:
                        ...
                        # Clickable circle button with invoice number

                        # Display format: top line (first 5 digits), bottom line (remaining)
                        ...
                        if inv_bottom:
                            ...
                            button_label = inv_top

                        if st.button(

                            button_label,

                            key=f"inv_{page}_{row_idx}_{col_idx}_{inv_num}",
...
<style>
    ...
    div[data-testid="stVerticalBlock"]:has(button[key="inv_{page}_{row_idx}_{col_idx}_{inv_num}"]) {{
