...
<style>
   ...
   /* Custom button styling for invoice cards */
...
def navigate_to_invoice(invoice_number):
   ...
   st.rerun()
# ------------------------------------------------------------
# Helper: Render single invoice card with clickable button
# ------------------------------------------------------------
def render_invoice_card(row, status_label, card_class, status_class, card_index):
   """Render a single invoice card with clickable invoice number."""
   ...
   with st.container():
       # Use columns for layout within the card
       col_left, col_right = st.columns([1, 1.2])
       with col_left:
           # Clickable invoice number button styled as circle
           if st.button(
               f"{inv_top}\n{inv_bottom}" if inv_bottom else inv_top,
               key=f"inv_btn_{card_index}_{inv_num}",
...
<div class="invoice-vendor">{vendor}</div>
       ...
       st.markdown(
           f"""
<style>
               div[data-testid="stVerticalBlock"]:has(button[key="inv_btn_{card_index}_{inv_num}"]) {{
                   background: {"linear-gradient(135deg, #fef2f2 0%, #fee2e2 100%)" if status_label == "Overdue" else "linear-gradient(135deg, #fffbeb 0%, #fef3c7 100%)" if status_label == "Disputed" else "linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)"};
                   border-radius: 16px;
                   padding: 1rem;
                   border: 1px solid #e5e7eb;
               }}
...
# ------------------------------------------------------------
# Helper: Render invoice card as styled container with button
# ------------------------------------------------------------
def render_invoice_card_styled(row, status_label, card_class, status_class, card_index):
   """Render invoice card matching the screenshot design."""
   inv_num = format_invoice_number(row["invoice_number"])
   inv_top, inv_bottom = split_invoice_number(row["invoice_number"])
   amt = abbr_currency(safe_number(row["amount"]))
   vendor = row["vendor_name"] if pd.notna(row["vendor_name"]) else "Unknown Vendor"
...
<div id="inv-circle-{card_index}" style="
                   background: #d1d5db;
                   border-radius: 50%;
                   width: 70px;
                   height: 70px;
                   ...
                   cursor: pointer;
               ">
<div style="font-size: 1rem; font-weight: 700; color: #111827; line-height: 1.2;">{inv_top}</div>
<div style="font-size: 1.2rem; font-weight: 700; color: #6b7280; line-height: 1.2;">{inv_bottom}</div>
</div>
<div style="text-align: right;">
<span class="invoice-status {status_class}">{status_label}</span>
<div class="invoice-amount" style="margin-top: 0.5rem;">{amt}</div>
...
<div class="invoice-vendor">{vendor}</div>
   ...
   # Add clickable button overlay for the invoice number
   if st.button(
       "🔍",
       key=f"inv_btn_{card_index}_{inv_num}",
...
def render_needs_attention(rng_start, rng_end, vendor_where):
   ...
   overdue_count = safe_int(cnt_df.loc[0, "overdue_count"]) if not cnt_df.empty else 32
...
""",
                   ...
                   # Clickable button to navigate to invoice
                   if st.button(
                       f"View Invoice {inv_num}",
                       key=f"inv_btn_{page}_{card_index}_{inv_num}",
                       use_container_width=True,
                       type="secondary",
                   ):
                       navigate_to_invoice(inv_num)
