...
<style>
   ...
   /* Invoice Circle - Non-selected (gray) */
   .invoice-circle {
       width: 80px;
       height: 80px;
       border-radius: 50%;
   ...
   /* Invoice Circle - Selected (blue) */
   .invoice-circle-selected {
       background: #3b82f6 !important;
   }
   .invoice-circle-selected:hover {
       background: #2563eb !important;
   }
   .invoice-circle .inv-top {
       font-size: 1.1rem;
       font-weight: 700;
       color: #111827;
       line-height: 1.2;
   }
   .invoice-circle .inv-bottom {
       font-size: 1.1rem;
       font-weight: 700;
       color: #6b7280;
       line-height: 1.2;
   }
   .invoice-circle-selected .inv-top,
   .invoice-circle-selected .inv-bottom {
   ...
   div[data-testid="column"] .circle-btn button {
   ...
   div[data-testid="column"] .circle-btn button:hover {
       background: #9ca3af !important;
       transform: scale(1.05);
   }
   div[data-testid="column"] .circle-btn-selected button {
       background: #3b82f6 !important;
       color: white !important;
   }
   div[data-testid="column"] .circle-btn-selected button:hover {
       background: #2563eb !important;
   }
</style>
""",
...
def format_invoice_number(invoice_num):
   ...
   return inv_str
# ------------------------------------------------------------
# Helper: Split invoice number for two-line display
# ------------------------------------------------------------
def split_invoice_for_display(invoice_num):
   """Split invoice number into two lines for circle display.
   Shows first 5 digits on top line, remaining on bottom line.
...
def navigate_to_invoice(invoice_number):
   ...
   st.rerun()
# ------------------------------------------------------------
# Helper: Render Invoice Card (matching screenshot layout exactly)
# ------------------------------------------------------------
def render_invoice_card_html(inv_num, inv_top, inv_bottom, status_label, status_class, amt, due_date, vendor, is_selected=False):
   """Render the HTML portion of an invoice card (without the button)."""
   # Determine card background
   if status_label == "Overdue":
       card_bg = "background: linear-gradient(135deg, #fef2f2 0%, #fee2e2 100%);"
   elif status_label == "Disputed":
       card_bg = "background: linear-gradient(135deg, #fffbeb 0%, #fef3c7 100%);"
   else:
       card_bg = "background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%);"
   # Circle styling
   circle_bg = "#3b82f6" if is_selected else "#d1d5db"
   text_color = "white" if is_selected else "#111827"
   bottom_color = "white" if is_selected else "#6b7280"
   # Build circle content
   if inv_bottom:
       circle_content = f"""
<div style="font-size: 1.1rem; font-weight: 700; color: {text_color}; line-height: 1.2;">{inv_top}</div>
<div style="font-size: 1.1rem; font-weight: 700; color: {bottom_color}; line-height: 1.2;">{inv_bottom}</div>
       """
   else:
       circle_content = f"""
<div style="font-size: 1.1rem; font-weight: 700; color: {text_color}; line-height: 1.2;">{inv_top}</div>
       """
   return f"""
<div style="{card_bg} border-radius: 20px; padding: 1rem; min-height: 140px;">
<div style="display: flex; align-items: flex-start; gap: 0.75rem;">
<div style="flex-shrink: 0;">
<!-- Circle placeholder for button -->
<div id="circle-placeholder-{inv_num}" style="width: 80px; height: 80px;"></div>
</div>
<div style="flex-grow: 1; text-align: right;">
<span class="status-badge {status_class}">{status_label}</span>
<div class="invoice-amount" style="margin-top: 0.5rem;">{amt}</div>
<div class="invoice-due-date" style="margin-top: 0.25rem;">Due: {due_date}</div>
</div>
</div>
...
def render_needs_attention(rng_start, rng_end, vendor_where):
   ...
   if attention_df.empty:
       attention_df = pd.DataFrame(
           [
               ...
               {"invoice_number": 9001234, "amount": 8500, "vendor_name": "ABB Ltd", "due_date": "2026-02-26"},
               {"invoice_number": 9002567, "amount": 12300, "vendor_name": "Siemens AG", "due_date": "2026-02-27"},
               {"invoice_number": 9003890, "amount": 5600, "vendor_name": "Rockwell Automation", "due_date": "2026-02-28"},
               {"invoice_number": 9004123, "amount": 9200, "vendor_name": "Parker Hannifin", "due_date": "2026-03-01"},
           ]
       )
       attention_df["due_date"] = pd.to_datetime(attention_df["due_date"])
   # Pagination - 8 items per page (2 rows of 4)
   items_per_page = 8
   ...
   for row_idx in range(2):
       row_start = row_idx * 4
       row_items = page_df.iloc[row_start:row_start + 4]
       if len(row_items) == 0:
           continue
       cols = st.columns(4)
       for col_idx, (df_idx, row) in enumerate(row_items.iterrows()):
           inv_num = format_invoice_number(row["invoice_number"])
           inv_top, inv_bottom = split_invoice_for_display(row["invoice_number"])
           ...
           with cols[col_idx]:
               ...
               with card_container:
                   ...
                   st.markdown(
                       f"""
<div style="background: {card_bg}; border-radius: 20px; padding: 1rem; min-height: 140px;">
...
""",
                   ...
                   button_key = f"inv_btn_{page}_{row_idx}_{col_idx}_{inv_num}"
                   # Apply circle styling based on selection
                   circle_bg = "#3b82f6" if is_selected else "#d1d5db"
                   text_color = "white" if is_selected else "#111827"
                   ...
                   st.markdown(
                       f"""
<style>
   div[data-testid="stButton"]:has(button[kind="secondary"][key="{button_key}"]) button {{
                           width: 75px !important;
                           height: 75px !important;
                           border-radius: 50% !important;
                           background: {circle_bg} !important;
                           border: none !important;
                       ...
                       div[data-testid="stButton"]:has(button[kind="secondary"][key="{button_key}"]) button:hover {{
                           background: {"#2563eb" if is_selected else "#9ca3af"} !important;
                           transform: scale(1.05);
                       }}
</style>
""",
                   ...
                   if st.button(button_label, key=button_key, use_container_width=False):
                       navigate_to_invoice(inv_num)
                   # Continue card HTML - right side with status, amount, due date
                   st.markdown(
                       f"""
</div>
<div style="flex-grow: 1; text-align: right;">
...
<div class="invoice-due-date" style="margin-top: 0.2rem;">Due: {due}</div>
</div>
</div>
<div style="margin-top: 0.4rem; padding-left: 0.25rem;">
<div class="invoice-vendor">{vendor}</div>
</div>
</div>
""",
   ...
   with col_prev:
       if st.button("â† Prev", disabled=(page == 0), use_container_width=True, key="na_prev"):
