...
# (Same as before, all INFORMATION_MART removed. Included but not repeated for brevity)
# ... (the full YAML from the previous answer is assumed to be here)
# For the sake of space, we include a placeholder. In the actual final code, the full YAML is present.
RAW_SEMANTIC_MODEL_YAML = """
name: "P2P Procure-to-Pay Analytics"
...
"""  # (Full YAML as in previous response)
...
def render_dashboard():
    ...
    if not attention_df.empty:
        ...
        # Custom CSS to make buttons look like cards
        st.markdown("""
        <style>
        .na-grid {
            display: grid;
        ...
        for i in range(0, len(page_df), 4):
            ...
            for j in range(4):
                if i + j < len(page_df):
                    ...
                    button_html = f"""
                    <div class="invoice-card-btn" style="--bg-color:{bg_color}; --border-color:{border_color}; background-color:{bg_color}; border:1px solid {border_color};">
                    ...
                    if cols[j].button(button_html, key=f"inv_card_{inv_num}_{i}_{j}", use_container_width=True):
                        # Navigate to Invoices page and set search term
...
# ---------------------------- GENIE PAGE (with clear on new clicks) ----------------------------
...
def render_invoices():
    ...
    # If an invoice search term was set from the dashboard, use it
    if "invoice_search_term" in st.session_state and st.session_state.invoice_search_term:
        st.session_state.inv_search_q = clean_invoice_number(st.session_state.invoice_search_term)
        # Clear it after using so it doesn't persist on subsequent visits
        del st.session_state.invoice_search_term
